...
const adItems = ref(null);

defineProps(['id']);

async function fetchData() {
    try {
        const response = await axiosApi.get(`/ads/${route.params.id}`);
...
<template>
    <div v-if="adItems" class="jm-card-border mb-2">
        <h5 class="card-title title-2 mb-2">{{ adItems.title }}</h5>
        ...
        <div class="card-body jm-shadow-box mt-3">
            <p class="px-3 py-2">{{ adItems.description }}
            </p>
        </div>
        ...
        <div class="mt-3 text-end">
            <SocNetworkItem v-if="$route.name === 'myAd'" />
        </div>
    </div>
    <div v-else>
        Veuillez réessayer...
